    ...
    **kwargs,
    ...
    try:
        peaks, peak_info = detect_ppg_peaks(
            ppg_signal, sampling_rate, method=peak_detection_method, **kwargs
        )
    ...
    try:
        time_points, hr_values = calculate_instantaneous_hr(
            peaks, sampling_rate, interpolation_rate=interpolation_rate, method="linear"
        )
    ...
    alpha = 0.2
    hr_values = signal.lfilter([alpha], [1, alpha - 1], hr_values)
