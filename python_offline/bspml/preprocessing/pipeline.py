...
def preprocess_ppg(
    ...
    1. Motion artifact removal (if accelerometer data available and motion detected)
    2. Detrending (baseline drift removal)
    3. Denoising (high-frequency noise removal)
    ...
    if enable_motion_removal and acc_signals is not None:
        ...
        if adaptive_motion_removal:
            ...
            print(f"Motion analysis: {high_motion_percentage:.1f}% above threshold {motion_threshold:.1f}")
            print(f"RLS filter: {'ENABLED' if should_use_rls else 'DISABLED'} (adaptive)")

        if should_use_rls:
            rls_params = preprocessing_params.get("rls", {})
            processed_signal = rls_filter(processed_signal, acc_resampled, **rls_params)
        else:
            print("RLS filter skipped due to low motion level")

    if enable_denoising:
        bandpass_params = preprocessing_params.get("bandpass", {})
        processed_signal = bandpass_filter(processed_signal, sampling_rate, **bandpass_params)
